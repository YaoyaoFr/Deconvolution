...
def RNN_Block_filter():
    ...
    # Optimizer
    loss_mse = tf.reduce_mean(tf.square(tf.sub(out_state_place, prediction), 0), name='loss_mse')
    ...
    with tf.Session() as sess:
        ...
        output = np.zeros([3001, batch_size, output_size])
        predicted_BOLD = np.zeros([3001, batch_size, BOLD_size])
        los = np.zeros([3001, output_size])

        # Test Variable
        test_output = np.zeros([3001, batch_size, output_size])
        test_predicted_BOLD = np.zeros([3001, batch_size, BOLD_size])
        test_los = np.zeros([3001, output_size])
        ...
        while True:
            ...
            # Test Model
            test_state = sess.run(rnn_cell.zero_state(batch_size=batch_size, dtype=tf.float64))
            for i in range(3001):
            ...
            test_predicted_neural = fun.move_average(test_predicted_neural, step_size=13)
            ...
            predicted_neural = fun.move_average(predicted_neural, step_size=13)

            # Show data by Show Type
            if show_type is 'train':
                los = np.mean(los, 0)
                error_string = 'Epoch - {:2d}  MSE:  s - {:10f}  f - {:10f}  v - {:10f} q - {:10f}  lr  {:10f} ' \
